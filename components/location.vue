<template>
    <Modal v-model="isMapPopShow" :closable="false" :mask-closable="false" title="坐标定位">
        <div class="map-wrap" style="height: 344px;position: relative; margin: -16px;margin-bottom: 0;">
            <i-input v-model="keyAdress" placeholder="请输入关键字，以便快捷定位" style="z-index:9;position: absolute;right: 20px;top:20px; width: 360px;" type="text"
                     v-on:input="inputFunc"></i-input>
            <i-select ref="associate" v-model="associate" style="z-index:5;position: absolute;right: 20px;top:20px;width: 360px;"
                      @on-change="getValue">
                <i-option v-for="item in associateList" :key="item.id" :value="item.id">
                    {{ item.name }}<b>{{ !item.address.length || item.address.length == 0 ? '' : item.address }}</b>
                </i-option>
            </i-select>
        ...
        <div slot="footer" class="opera-box">
            <i-button @click="handlePopCancle">取消</i-button>
...
<script>
import mapUtils from "../js/AMap-utils.js";
